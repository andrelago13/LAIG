...
function TransformMatrix(transforms) {
	...
	for(var i = 0; i < transforms.length; i++) {
		...
		case "TRANSLATION":
			this.matrix = mat4.translate(this.matrix, 
					this.matrix, 
		...
		case "ROTATION":
			...
			if(axis == 'x')
				this.matris = mat4.rotateX(this.matrix, this.matrix, transform["angle"]);
			else if(axis == 'y')
				this.matris = mat4.rotateY(this.matrix, this.matrix, transform["angle"]);
			else if(axis == 'z')
				this.matris = mat4.rotateZ(this.matrix, this.matrix, transform["angle"]);
